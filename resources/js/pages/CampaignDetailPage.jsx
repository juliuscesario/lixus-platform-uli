...
export default function CampaignDetailPage({ user }) {
    ...
    // Use the authenticated user or the passed user prop
    ...
    // State for apply modal and process
    ...
    useEffect(() => {
        const fetchCampaignDetail = async () => {
            ...
            try {
                // Use getCampaignDetail which is a public endpoint
                const response = await api('getCampaignDetail', id);
                ...
                setError(err.message || "Tidak dapat memuat detail kampanye.");
            } finally {
                setLoading(false);
            }
        };
    ...
    if (error) {
        return (
            <div className="max-w-4xl mx-auto py-12 px-4 text-center text-red-500 bg-red-100 p-4 rounded-md">
                {error}
            </div>
        );
    }
    ...
    const isBrandOrAdmin = currentUser && (currentUser.role === 'brand' || currentUser.role === 'admin');
    ...
    const renderActionButton = () => {
        if (!currentUser) {
            return (
                <Link 
                    to="/login" 
                    className="bg-pink-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-pink-700 transition-colors duration-300 text-lg"
                >
        ...
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
                <img 
                    ...
                    src={campaign.image || `https://placehold.co/800x400/f472b6/ffffff?text=${encodeURIComponent(campaign.name)}`} 
                    alt={campaign.name} 
                />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 border-t border-b border-gray-200 py-6">
                        {currentUser && (
                            <div>
                                ...
                                <p className="text-lg font-bold text-gray-800 capitalize">{campaign.status}</p>
                            </div>
                        )}
                        {isBrandOrAdmin && (
                            <div>
                                <p className="text-sm font-semibold text-gray-500">Budget</p>
                                <p className="text-lg font-bold text-gray-800">{formatCurrency(campaign.budget)}</p>
                            </div>
                        )}
                        <div>
                            <p className="text-sm font-semibold text-gray-500">Tanggal Mulai</p>
                            <p className="text-lg font-bold text-gray-800">{formatDate(campaign.start_date)}</p>
                        </div>
                        <div>
                            <p className="text-sm font-semibold text-gray-500">Tanggal Selesai</p>
                    ...
                    <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                        <div className="mt-3 text-center">
                            ...
                            <p className="text-sm text-gray-500 mb-6">
                                Apakah Anda yakin ingin mendaftar untuk kampanye "{campaign.name}"?
                            </p>
                            <div className="flex justify-center space-x-3">
                                ...
                                <button
                                    ...
                                    disabled={isApplying}
                                >
                                    {isApplying ? 'Mendaftar...' : 'Ya, Daftar'}
                                </button>
                            </div>
                        </div>
                    </div>
